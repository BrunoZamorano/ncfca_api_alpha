...
describe('Create Club', () => {
  ...
  beforeEach(() => {
    ...
    const userFactory = {
      create(props: CreateUserProps): User {
        return User.create(props, idGenerator, hashingService);
      },
    };
  ...
  it('Deve criar um clube', async function () {
    ...
    expect(output.principalId).toBe(input.loggedInUserId);
    ...
    expect(club.principalId).toBe(input.loggedInUserId);
