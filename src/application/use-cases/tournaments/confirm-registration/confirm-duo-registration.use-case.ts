...
@Injectable()
export class ConfirmDuoRegistration {
  constructor(
    @Inject(EVENT_EMITTER_FACADE) private readonly eventEmitterFacade: EventEmitterFacade,
    @Inject(UNIT_OF_WORK) private readonly unitOfWork: UnitOfWork,
  ) {}

  async execute(input: ConfirmDuoRegistrationCommand): Promise<void> {
    return await this.unitOfWork.executeInTransaction(async () => {
      ...
      if (!tournament) {
        throw new EntityNotFoundException('Tournament', `with registration ${input.registrationId}`);
      }

      tournament.confirmDuoRegistration(input.registrationId, this.eventEmitterFacade.tournamentEmitter);
      await this.unitOfWork.tournamentRepository.save(tournament);
    });
  }
}

export interface ConfirmDuoRegistrationCommand {
